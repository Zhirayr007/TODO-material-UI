...
export class TasksStore {
  ...
  getTasks = async (searchParams?: SearchFormEntity) => {
    ...
    //Для исправления бага связанный с отсутствием isCompleted у некоторых task-ов
    for (const key in res) {
      if (res[key].isCompleted == undefined) {
        await TaskAgentInstance.updateTask(String(res[key].id), {
          isCompleted: false,
        });
      }
    }
